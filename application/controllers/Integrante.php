	...
	*/
	...
	public function listado($idencuesta = NULL){		
		...
		if (isset($idencuesta) && $idencuesta > 0){
			...
			//$data['lstintegrantes'] = $this->encuesta_model->get_familia($idencuesta);
...
/*	public function nueva($idencuesta){
		...
		if (isset($idencuesta) && $idencuesta > 0){
			...
			$this->load->model('filialempresa_model');
			$data['detfilempresa'] = $this->filialempresa_model->get_filialempresa_by_id($idencuesta);
			...
			if ($this->input->post('hdn_filempresaid') != "" && $this->input->post('hdn_filempresaid') > 0){
				...
				$apellido_m = trim($apellido_m) != "" ? $apellido_m : NULL;
				...
				$this->form_validation->set_rules('txt_apellido_p','Apellido Paterno','required|min_length[2]|max_length[50]');
				$this->form_validation->set_rules('txt_apellido_m','Apellido Materno','min_length[2]|max_length[50]');				
				$this->form_validation->set_rules('txt_fec_nacimiento','Fecha de nacimiento','required');
				$this->form_validation->set_rules('sel_nac_chilena','Nacionalidad chilena','required');
				$this->form_validation->set_rules('sel_genero','Género','required');
				...
				if ($this->form_validation->run() == FALSE){
					...
					$this->integrante_model->agregar_integrante($encuesta_familia);
					$data['lstencuestas'] = $this->encuesta_model->get_encuestas_by_usuario_filialempresa($sesionusuario['usrid'],$idfilempresa);

					$data['mensaje'] = "El integrante ha sido agregado exitosamente";
					$data['divtipo'] = "alert alert-success alert-dismissable";
