...
class Encuesta extends CI_Controller {
		...
		if (isset($idencuesta) && $idencuesta > 0){
			...
			if ($this->input->post('hdn_encuestaid') != "" && $this->input->post('hdn_encuestaid') > 0){
				...
				if ($this->form_validation->run() == FALSE){
					...
					$data['divtipo'] = "alert alert-danger alert-dismissable";
					$this->load->view('recopilador/header',$data);
					$this->load->view('encuesta/familia',$data);
					$this->load->view('recopilador/footer',$data);
				}
				else{
					...
					if($verificador!=0){
						...
						$this->encuesta_model->crear_encuesta_familia($encuesta_familia);			
	...
	/*public function familia($idencuesta,$integrante){
		...
		if (isset($idencuesta) && $idencuesta > 0){
			...
			$verificador = $this->encuesta_model->verificar_registro_familia($idencuesta);
			
			if ($verificador > 0){
				
				$familia = $this->encuesta_model->get_familia_by_id($idencuesta);		
			...
			if ($this->input->post('hdn_encuestaid') != "" && $this->input->post('hdn_encuestaid') > 0){
				...
				if ($this->form_validation->run() == FALSE){
					...
					if($verificador!=0){
						$this->encuesta_model->actualizar_encuesta_familia($encuesta_familia);
						//$data['lstencuestas'] = $this->encuesta_model->get_encuestas_by_usuario_filialempresa($sesionusuario['usrid'],$idencuesta);
